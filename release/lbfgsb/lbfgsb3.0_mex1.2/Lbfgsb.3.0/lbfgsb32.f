...
c
c       Linpack Library ... dcopy32, ddot32.
c
c
c     References:
c
...
c     ************
     ...
     +                 iword,nfree,nact,ileave,nenter
      double precision theta,fold,ddot32,dr,rr,tol,
...
c                                          restore the previous iterate.
               call dcopy32(n,t,1,x,1)
               call dcopy32(n,r,1,g,1)
               f = fold
            endif
            goto 999
         endif
...
c
c       Linpack ... dscal32 dcopy32, daxpy32.
c
c
c     References:
c
...
c     ************
     ...
     +                 tu,tl,wmc,wmp,wmw,ddot32,tj,tj0,neggi,sbgnrm,
     +                 f2_org
      double precision one,zero
      parameter        (one=1.0d0,zero=0.0d0)
...
c                   complete the initialization of p for theta not= one.
         call dscal32(col,theta,p(col+1),1)
      endif
...
c           compute (wbp)Mc, (wbp)Mp, and (wbp)M(wbp)'.
         ...
         if (info .ne. 0) return
         wmc = ddot32(col2,c,1,v,1)
         wmp = ddot32(col2,p,1,v,1) 
         wmw = ddot32(col2,wbp,1,v,1)
 
c           update p = p - dibp*wbp. 
         call daxpy32(col2,-dibp,wbp,1,p,1)
...
c       which will be used in computing r = Z'(B(x^c - x) + g).
      ...
      if (col .gt. 0) call daxpy32(col2,dtm,p,1,c,1)
      if (iprint .gt. 100) write (6,1010) (xcp(i),i = 1,n)
...
c     ************
     ...
     +                 col2,pbegin,pend,dbegin,dend,upcl
      double precision ddot32,temp1,temp2,temp3,temp4
      double precision one,zero
      parameter        (one=1.0d0,zero=0.0d0)
...
c                                 shift old part of WN1.
            do 10 jy = 1, m - 1
               js = m + jy
               call dcopy32(m-jy,wn1(jy+1,jy+1),1,wn1(jy,jy),1)
               call dcopy32(m-jy,wn1(js+1,js+1),1,wn1(js,js),1)
               call dcopy32(m-1,wn1(m+2,jy+1),1,wn1(m+1,jy),1)
  10        continue
         endif
...
c        upper triangle of (2,2) block of wn.
      ...
      do 72 is = col+1, col2
         do 74 js = is, col2
               wn(is,js) = wn(is,js) + ddot32(col,wn(1,is),1,wn(1,js),1)
  74        continue
  72     continue
...
c     Determine the maximum step length.
  ...
  43        continue
      ...
      call dcopy32(n,x,1,t,1)
      call dcopy32(n,g,1,r,1)
      fold = f
      ifun = 0
      iback = 0
      csave = 'START'
...
c     Update matrices WS and WY.
      ...
      call dcopy32(n,d,1,ws(1,itail),1)
      call dcopy32(n,r,1,wy(1,itail),1)
...
c                              move old information
         do 50 j = 1, col - 1
            call dcopy32(j,ss(2,j+1),1,ss(1,j),1)
            call dcopy32(col-j,sy(j+1,j+1),1,sy(j,j),1)
  50     continue
      endif
...
c                                             and the last column of SS:
      pointr = head
      do 51 j = 1, col - 1
         sy(col,j) = ddot32(n,d,1,wy(1,pointr),1)
         ss(j,col) = ddot32(n,ws(1,pointr),1,d,1)
         pointr = mod(pointr,m) + 1
  51  continue
      if (stp .eq. one) then
         ss(col,col) = dtd
...
c     Compute d = (1/theta)d + (1/theta**2)Z'W wv.
  ...
  40  continue
      ...
      call dscal32( nsub, one/theta, d, 1 )
c 
c-----------------------------------------------------------------
